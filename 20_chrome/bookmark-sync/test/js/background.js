...
function getRootBookmarks(arr, id) {
    let newArr = [];
...
function emptyBookmarks(bookmarkTreeNodeArray) {
    var bookmarks = bookmarkTreeNodeArray[0].children[0].children;
    for (let i of bookmarks) {
        try{
            chrome.bookmarks.removeTree(i.id, null);
        }catch (e) {
            console.log("同步期间修改了书签");
        }
    }
}

function createBookmark(arr, id, newid) {
    let newArr = getRootBookmarks(arr, id);
    if (newArr.length == 0) {
        return;
    }
    for (let [index, i] of newArr.entries()) {
        chrome.bookmarks.create({
            ...
            if (bookmarkTreeNode) {
                createBookmark(arr, i.id, bookmarkTreeNode.id);
            }
        });
    }
}
...
var timer = function () {
    if (localStorage.token != undefined && localStorage.token != null && localStorage.token != "") {
        chrome.bookmarks.getTree(function (bookmarkTreeNodeArray) {
            $.ajax({
                type: 'POST',
                url: "https://api.icql.work/bookmark/sync",
                data: {
                    "token": localStorage.token,
                    "version": localStorage.bookmarkversion,
                    "bookmarkJson": JSON.stringify(bookmarkTreeNodeArray)
                },
                dataType: "json",
                success: function (result) {
                    switch (result.code) {
                        case 20000:
                            break;
                        case 20001:
                            localStorage.bookmarkversion = result.data.version;
                            let bookmarks = result.data.treeNodeDTOS;
                            emptyBookmarks(bookmarkTreeNodeArray);
                            createBookmark(bookmarks, "1", "1");
                            break;
                        case 20002:
                            localStorage.bookmarkversion = result.data;
                            break;
                        default:
                    }
                },
                error: function (result) {
                    ...
                    if(result.status == 401){
                        $.ajax({
                            ...
                            data: {
                                "account": localStorage.nameOrEmail,
                                "password": Base64.decode(localStorage.password).substring(10),
                                "expire": -1
                            },
                            dataType: "json",
                            success: function (result) {
    ...
    if (localStorage.timeInterval == "") {
        ...
        timeInterval = 1000 * 60 * parseInt(localStorage.timeInterval);
    }
    setTimeout(timer, timeInterval)
}
timer();
